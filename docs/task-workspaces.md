...
Pending task entries in the worker pool carry dispatch constraints separately from the worker-facing task payload. The current constraint is the workflow instance host pin. This lets the orchestrator preserve placement requirements while work is waiting in memory, then apply claim-time matching before a worker receives the task.
