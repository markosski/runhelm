...
RunHelm currently provides at-least-once task execution. Workflow state is protected from stale worker results, but task execution itself may be retried or duplicated after orchestrator restart, lease timeout, worker loss, verifier rerun, or explicit retry.

If the orchestrator restarts before a worker reports a task result, the restarted orchestrator begins with an empty worker registry and no in-memory dispatch leases. Startup recovery treats any durable `Running` workflow or task attempt as abandoned, moves it back to runnable state, and queues it for a fresh engine pass. Post-restart dispatch IDs include a fresh worker-pool namespace so abandoned pre-restart results do not collide with newly recovered dispatches. For the initial remote-worker implementation, a result from a worker that has not rejoined is rejected as unregistered and does not advance workflow state. If the worker re-registers but reports the old dispatch ID, the result is still late or untracked unless it matches a currently active fresh dispatch. This keeps workflow state consistent, but the old worker and a newly dispatched worker may both execute the same logical task attempt after restart. Side-effecting tasks should therefore be idempotent until durable lease reattachment exists. A future durable lease-backed result path may accept such results only when the result can be matched to a valid persisted dispatch lease.
