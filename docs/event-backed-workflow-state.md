...
- `POST /workflows/{id}/pause` records a `WorkflowStatusChanged` event to move a `Pending` or `Running` workflow to `Paused` and removes that workflow from the pending execution queue. Repeating the request for an already paused workflow is accepted as a no-op. Terminal workflows and workflows waiting for human input reject pause with conflict behavior.
- `POST /workflows/{id}/resume` records a `WorkflowStatusChanged` event to move a paused workflow back to `Pending` and queues it for execution.
