...
- `GET /workflows/{id}/tasks` and `GET /workflows/{id}/tasks/{task_id}` report task attempts waiting for human input with `status: "input_needed"` and the input request `description`; they are not reported as `running`.
