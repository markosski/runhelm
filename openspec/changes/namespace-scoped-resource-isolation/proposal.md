...
- Scope all definition, workflow-instance, task, event, queue, recovery, reconciliation, dispatch, and result operations by namespace.
- Update memory, SQL, and AWS storage identities and queries so the same resource identifier can exist independently in multiple namespaces.
...
This affects the public API router and handlers, workflow and function services, orchestrator and engine, queue and task-dispatch contracts, startup/background reconciliation, worker endpoints, every storage adapter, the SQL initial schema, AWS DynamoDB/S3 keys and projections, tests, OpenSpec requirements, and developer-facing configuration and migration documentation. Public definition bodies remain unchanged because namespace is derived from request context rather than user-controlled resource payloads.
